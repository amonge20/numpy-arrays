...
# Datos de ventas de la tienda (fecha de la venta, monto, )
ventas = np.array([
    ['2022-01-01', 100, 'ropa'],
...
# Filtramos los meses
meses = np.array([fecha[5:7] for fecha in ventas[:,0]])

# Se convierte los montos en enteros
montos = ventas[:,1].astype(int)

# Recorremos otro bucle para la suma del precio de las ventas despues de que se filtren por mes
for mes in np.unique(meses):
    total = montos[meses == mes].sum()
